...
进程切换，似乎不能够换回刚刚运行的位置，我在test_a一开始加了printf，结果每次都会输出。

![](figure/2018-04-16-14-32-31.png)

di寄存器的取值不正确。

![](figure/2018-04-16-14-33-19.png)

这里不应该加方括号



## bug：在汇编中访问结构体中指定元素的坑

关于结构体元素自动对齐的问题。

![](figure/2018-04-16-16-25-36.png)


### bug:不知道为什么


运行了一段时间之后，本来好好地两个进程并发执行突然卡住。
![](figure/2018-04-16-17-03-43.png)

如下图，发现每次运行，栈指针都会减2！

![](figure/2018-04-16-18-23-38.png)

再来看看云心restart函数的时候
![](figure/2018-04-16-18-24-42.png)
push sp 这一条指令中，机器的实现是先push在把sp减2.
这与我以往的认知：先减2再push不同。

很早就听说过这是一条机器相关的指令，但是被坑的这么惨还是第一次。
![](figure/2018-04-16-18-26-10.png)
如此替换为无歧义的指令之后，终于好了！！