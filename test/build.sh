...
gcc -c -m16 -o c.o t.c || { echo "GCC complied failed"; exit 1; }
nasm -f elf32 -o asm.o t.asm || { echo "NASM complied failed"; exit 1; }
ld -o test.bin -Ttext 0x10000 -m elf_i386 -e main --oformat binary c.o asm.o || { echo "LINK failed"; exit 1; }
...
dd if=test.bin of=a.img ibs=512 count=10 conv=notrunc seek=18
...
# dd if=test.bin of=a.img ibs=512 count=10 conv=notrunc seek=108
